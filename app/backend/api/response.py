...
@router.post('/responses/vacancies/{vacancy_id}', tags=['Response'], dependencies=[Depends(response_limiter)])
...
@router.get('/responses/vacancies/{vacancy_id}', response_model=list[ResponseRead], tags=['Response'])
...
@router.put('/responses/{response_id}/status', tags=['Response'], dependencies=[Depends(set_status_limiter)])
